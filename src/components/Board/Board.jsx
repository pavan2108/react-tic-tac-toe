...
function Board({ setScores, option }) {
  ...
  const handleClick = (id) => {
    ...
    let randomNumber;
    if (option === "pro") {
        randomNumber = getRandom(getAvailableMoves(editedBoard));
    }
    else {
        randomNumber = getBestMove(editedBoard, 0, false);
    }
  ...
  return (
    <div className="board">
      ...
      <button onClick={gameReset} className="button">Reset</button>
    </div>
  );
}
